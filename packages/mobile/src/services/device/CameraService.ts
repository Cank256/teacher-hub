...
export class CameraService {
    ...
    async takePhoto(options: CameraOptions = { mediaType: 'photo', quality: 'medium' }): Promise<CameraResult | null> {
        try {
            ...
            if (options.aspect) {
                cameraOptions.aspect = options.aspect
            }

            if (options.maxDuration) {
                cameraOptions.videoMaxDuration = options.maxDuration
            }
            ...
            if (asset.width != null) {
                cameraResult.width = asset.width
            }
            if (asset.height != null) {
                cameraResult.height = asset.height
            }
            if (asset.duration != null) {
                cameraResult.duration = asset.duration
            }
            if (asset.fileName != null) {
                cameraResult.fileName = asset.fileName
            }
    ...
    async pickFromLibrary(options: CameraOptions = { mediaType: 'photo', quality: 'medium' }): Promise<CameraResult | null> {
        try {
            ...
            if (options.aspect) {
                libraryOptions.aspect = options.aspect
            }
